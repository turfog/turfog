  ...
  const card = (p: AvailablePlayer) => (
    <div className="bg-white rounded-2xl border border-neutral-200 shadow-card p-3.5 h-full flex flex-col">
      <div className="flex items-start gap-2.5">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-1">
            <Link href={`/${p.username}`} className="text-body-xs font-semibold text-neutral-900 hover:text-electric-blue truncate">
              {p.name}
            </Link>
            {p.verified && <CheckCircleIcon size={13} className="text-electric-blue flex-shrink-0" />}
          </div>
          <Link href={`/${p.username}`} className="text-caption text-neutral-400 hover:text-electric-blue truncate block">
            @{p.username}
          </Link>
        </div>
      ...
      <div className="flex items-center gap-2 mt-3">
        <Link
          href={`/${p.username}`}
          className="flex-1 inline-flex items-center justify-center gap-1.5 px-3 py-2 rounded-xl text-body-xs font-semibold bg-primary-green text-white hover:bg-primary-green/90 transition-colors"
        >
          View profile
        </Link>
        <Link
          href="/games"
          className="px-3 py-2 rounded-xl border border-neutral-200 text-neutral-500 text-body-xs font-medium hover:bg-neutral-50 inline-flex items-center gap-1 transition-colors"
        >
          Invite
  ...
  return (
    <section>
      <div className="flex items-center justify-between mb-3">
        ...
        <span className="px-2 py-0.5 rounded-full bg-emerald/10 text-caption font-semibold text-emerald">
          {heartbeats.length} live
        </span>
      </div>
      {variant === "rail" && (
        <p className="text-caption text-neutral-400 -mt-1 mb-3">Players ready to play near you</p>
      )}
      ...
      {loading ? (
        ...
        <div className="flex gap-3 overflow-x-auto scrollbar-hide pb-1 -mx-1 px-1">
          {heartbeats.map((p) => (
            <div key={p.id} className="w-[268px] flex-shrink-0">
              {card(p)}
            </div>
          ))}
        </div>
